...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:GetFlagEffect(id)==0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x945,0x11,700,1100,3,RACE_PYRO,ATTRIBUTE_FIRE) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x945,0x11,700,1100,3,RACE_PYRO,ATTRIBUTE_FIRE) then
		c:AddMonsterAttribute(TYPE_NORMAL)
		c:AssumeProperty(ASSUME_RACE,RACE_PYRO)
