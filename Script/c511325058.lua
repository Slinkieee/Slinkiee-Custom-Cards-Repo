...
function s.shfilter(c)
	return c:IsSetCard(0x945) and c:IsType(TYPE_SPELL) and c:IsAbleToHand() and not c:IsCode(id)
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:GetFlagEffect(id)==0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x945,0x11,1100,700,3,RACE_THUNDER,ATTRIBUTE_WIND) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x945,0x11,1100,700,3,RACE_THUNDER,ATTRIBUTE_WIND) then
