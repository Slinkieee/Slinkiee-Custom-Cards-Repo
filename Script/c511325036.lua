--Fenjou Nether
local s,id=GetID()
function c511325036.initial_effect(c)
	--link summon
	c:EnableReviveLimit()
...
function s.lfilter(c,lc,sumtype,tp)
	return c:IsSetCard(0x944,lc,sumtype,tp) and c:IsAttribute(ATTRIBUTE_WIND,lc,sumtype,tp)
end
...
function s.penfilter(c,e,tp,lscale,rscale)
	return c:IsSetCard(0x944) and Pendulum.Filter(c,e,tp,lscale,rscale)
end
